...
type_of_sensor_fit <- "linear" ## or "quadratic"
sensor_concentrations <- c(4, 10, 18)
conversion_factors <- c(4.165e-5, 0.0001428, 0.00027693)
base_fluorescences <- c(5565, 4347, 2959)
linear_slope <- c(1603.245, 1910.982, 1875.960)
linear_intercept <- c(1016.848, 1968.054, 3217.200)
conversion_table <- data.frame(sensor_concentrations, conversion_factors, base_fluorescences, linear_slope, linear_intercept)
#conversion_table <- mutate(conversion_table, linear_base_fluorescence = (base_fluorescences - linear_intercept)/linear_slope)
conversion_table <- mutate(conversion_table, quadratic_base_fluorescence = base_fluorescences * conversion_factors)
#fluorescence.at.2uM.Pi <- 27173.39 - base_fluorescences[1]
...
#data<-cbind(data, paste(data$date, data$sample, data$conc, datadata$GAP_conc, data$well, sep = " "))
...
for ( i in seq_along(proteins) ) {
  ...
  pdf(plot_filename)
  for ( j in seq_along(sensor_concentrations) ) {
    data_prot_sensor <- data[data$sample == proteins[i] & data$sensor_conc == sensor_concentrations[j], ]
    if ( length(data_prot_sensor[ ,1]) > 0 ) {
      samples <- unique( as.character(data_prot_sensor$conc) )
      col_list <- list( samples, rainbow(length(samples)) )
      plot(data_prot_sensor$time, data_prot_sensor$fluorescence, xlab = "time / s", ylab = "fluorescence", type = "n") #, ylim = c(5000, 40000))
      for ( k in seq_along(samples) ) {
        temp <- data_prot_sensor[ data_prot_sensor$conc == samples[k], ]
...
get_linear_region <- function(data) {
  ...
  reaction_percentages <- seq(0.1, 0.3, 0.05)
  points_check <- TRUE
...
# fit_one_phase_association <- function (data, sensor_conc) {
#   base_fluorescence <- conversion_table[["linear_base_fluorescence"]][conversion_table[["sensor_concentrations"]] == sensor_conc]
#   opar <- par()
#   op<-par(mfrow=c(2,1))
...
#   converted_rate <- v0 * conversion_table[["conversion_factors"]][conversion_table[["sensor_concentrations"]] == sensor_conc]
#   linear_intercept <- conversion_table[["linear_intercept"]][conversion_table[["sensor_concentrations"]] == sensor_conc]
#   linear_slope <- conversion_table[["linear_slope"]][conversion_table[["sensor_concentrations"]] == sensor_conc]
#   linearly_converted_rate <- (v0 - linear_intercept)/linear_slope
#   return(data.frame("rate constant" = K, "initial_rate" = v0, converted_rate, linearly_converted_rate ))
#   par(opar)
# }


fit_linear_rate <- function (data, sensor_conc, GAP_conc) {
  ...
  time <- unique(relevant.subset$time)
  main = paste(data$sample[1], data$date[1], data$corrected_conc[1], data$GAP_conc[1], sep = " ")
  #y.axis.lim <- c(floor(base_fluorescence/10000)*10000, max(data$con_fluorescence, na.rm = T))
  y.axis.lim <- c(min(data$con_fluorescence), max(data$con_fluorescence, na.rm = T))
  plot(data$time, data$con_fluorescence, xlab = "time / s", ylab = "converted fluorescence", pch = 20, main = main, ylim = y.axis.lim)
  points(relevant.subset$time, relevant.subset$con_fluorescence, pch = 20, col = "red")
  lm.out <- lm(con_fluorescence ~ time, data = relevant.subset)
  abline(lm(con_fluorescence ~ time, data = relevant.subset))
  #predicted.line.data.frame <- abline(lm(con_fluorescence ~ time, data = relevant.subset))
...
fit_MichaelisMenten <- function (initial_vs_conc, prot) {
  ...
  getFit <- function(pars, xx) (pars$kcat * conc) / (pars$Km + conc)
  ...
  parStart <- list(kcat = kcat.estimate, Km = Km.estimate)
  ...
  getPred <- function(pars, xx) (pars$kcat * test.conc) / (pars$Km + test.conc)
  ...
  parameters_and_predicted_line <- list("parameters" = data.frame(prot, Km, kcat, kcat_error, Km_error, sd_of_fit), 
      ...
      for (g in seq_along(GAP_concentrations)) {
        ...
        for (e in seq_along(experiments)) {
          ...
          for (w in seq_along(wells)) {
            well <- wells[w]
            data.subset <- data[data$sample == prot & data$sensor_conc == sensor_conc & data$corrected_conc == conc &
                                  data$GAP_conc == GAP_conc & data$date == exp & data$well == well,]
            if ( length(data.subset[ ,1]) > 0 ) {
              data.subset <- data.subset[order(data.subset$time),]
              if (type_of_sensor_fit == "linear") {
                linear_intercept <- conversion_table$linear_intercept[conversion_table$sensor_concentrations == sensor_conc]
                linear_slope <- conversion_table$linear_slope[conversion_table$sensor_concentrations == sensor_conc]
                data.subset <- mutate(data.subset, con_fluorescence = (fluorescence - linear_intercept) / linear_slope)
                #base_fluorescence <- conversion_table$linear_base_fluorescence
                rates <- fit_linear_rate(data.subset, sensor_conc, GAP_conc)
                #rates <- fit_one_phase_association(data.subset, sensor_conc, GAP_conc, base_fluorescence)
              } else if (type_of_sensor_fit == "quadratic") {
                conversion_factor <- conversion_table$conversion_factors[conversion_table$sensor_concentrations == sensor_conc]
                data.subset <- mutate(data.subset, con_fluorescence = fluorescence * conversion_factor)
                base_fluorescence <- conversion_table$quadratic_base_fluorescence
                rates <- fit_linear_rate(data.subset, sensor_conc, GAP_conc, base_fluorescence)
                #rates <- fit_one_phase_association(data.subset, sensor_conc, GAP_conc, base_fluorescence)
              }
              rates <- cbind(rates, data.frame(prot, conc, sensor_conc, GAP_conc, exp, well))
...
fitting_parameters <- cbind(fitting_parameters, data.frame(
  "v0" = ifelse(fitting_parameters$GAP_conc > 0,
              fitting_parameters$initial_rate / (fitting_parameters$GAP_conc/1000),
              fitting_parameters$initial_rate)
  ))
...
#filename <- paste0("GTPase_assay/MichaelisMenten_", Sys.Date(), "_", type_of_sensor_fit, ".pdf")
