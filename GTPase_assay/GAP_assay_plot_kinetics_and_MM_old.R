...
setwd("GTPase_assay")
conversion_factor = 2.977e-05 # uM / fluorescence
base_fluorescence = 7123
fluorescence.at.2uM.Pi <- 33882
...
max_fluorescence_per_sample <- data.frame("sample" = as.character(unique(data$sample)), "x" = fluorescence.at.2uM.Pi) 
data_low_uM <- data[data$conc == 2,]
max_fluorescence_at_low_uM <- with(data_low_uM, aggregate(fluorescence, by = list(sample = sample), max))
fluorescence_ratio <- merge(max_fluorescence_per_sample, max_fluorescence_at_low_uM, by = "sample")
...
get_region_to_fit <- function (data) {
 ...
 cutoff_time <- plateau_times[1] + 100
    ...
    for (g in seq_along(GAP_concentrations)) {
      ...
      for (e in seq_along(experiments)) {
        ...
        for (w in seq_along(wells)) {
          ...
          #rates <- fit_two_phase_association(data.subset)
