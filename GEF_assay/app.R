...
# load data
#load('../cjm_data.Rda')
dataset <- read_tsv("good_data_parsed.txt")
...
for (samp in unique(unlist(dataset$sample))) {
  ...
  if(!file.exists(paste0(output, samp, '/', samp, '.tsv'))) {
    data.to.write <- dataset %>%
      filter(sample == samp) %>%
      filter(Time == 0) %>%
      select(-Time, -fluorescence)
    
    data_columns <- c('chisq','pearsonr','max_flo', 'f_plateau', 'span1','span1_pval', 'span2', 'k','k_pval', 'k_background','f_mid', 'f0', 'vf0')
    data.to.write[,data_columns] <- 0.0
    
    write_delim(data.to.write, path = paste0(output, samp, '/', samp, '.tsv'), delim = '\t', na = 'NA')
    }
}
...
save_and_print <- function(d) {
  ...
  row.to.save <- d %>%
    select(chisq, pearsonr, conc, cutoff_time, max_flo, f_plateau, span1, span1_pval, span2, k, k_pval, k_background, f_mid, f0, vf0) %>%
    head(1)
