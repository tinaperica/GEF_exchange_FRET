...
data <- read.delim("GEF_assay/data/GEF_FRET_kinetics.txt", head = F)
...
data$unique <- do.call(paste, c(data[, c("date", "sample", "conc", "GEF_conc", "well")], sep = " "))
data <- data[data[["conc"]] > 0,]
proteins <- as.character(unique(data$sample))
for ( i in seq_along(proteins) ) {
  plot_filename <- paste0("GEF_assay/", Sys.Date(), "_", proteins[i], "_GEF_assay.pdf")
  ...
  legend("top", legend = col_list[[1]], title = "Ran/uM and GEF/nM", fill = col_list[[2]], bty = "n", cex = 1)
  dev.off()
}
