...
run_nls <- function(data, deadtime, debug = FALSE) {
  ...
  start <- list(f0 = max(data$observed, na.rm = T),
                ...
                k_background = 1e-4)  
  lower <- c(0.0,
             0.8 * min(data$observed, na.rm = T),
             0.0,
             0.0,
             1e-6)
  upper <- c(2*max(data$observed, na.rm = T),
             1.2 * min(data$observed, na.rm = T),
  ...
  data$exchange <- span1 * (exp(-k * data$Time))  + f_plateau
  data$exchange_norm <- c0* (exp(-k * data$Time)) 
  data$background <-  (f0 - f_plateau - span1) * exp(-k_background * data$Time)
...
plot_data <- function(data, debug = FALSE) {
  ...
  pdf(paste(today, 'data.pdf', sep = '_'))
  ...
  # for (i in seq_along(conditions)) {
  #   data_to_plot <- data %>% filter(condition == conditions[i])
  #   plots[[i]] <- ggplot(data_to_plot, mapping = aes(x = Time, y = exchange_conc)) + geom_point(color = "black") +
  #     geom_line(data_to_plot, mapping = aes(x = Time, y = exchange_conc), color = "cyan") +
  #     ggtitle(conditions[i])
  # }
  # pdf(paste(today, 'exchange_conc.pdf', sep = '_'))
  # 
  # print(plots)
  # dev.off()
  ...
  if (debug) {
    ...
    print(ggplot(data, aes(x = conc, y = 1000*k, color = as.character(date))) + geom_point())
...
#   select(sample, date, conc, condition, span1, f0, k, k_background, f_plateau, vf0, f_signal_change) %>% 
#   unique() %>%
#   arrange(sample, conc)
# fit.parameters %>% filter(vf0 < 10 & vf0 > 0) %>%
#   ggplot(., aes(conc, vf0, color = as.character(sample))) + geom_point()


#ggsave("~/Desktop/temp.pdf")

data.exp <- plot_data(data.exp, debug = F)

# 
# data.exp %>%
#   filter(condition == '20180205-PE1_WT-A1-1.75') %>%
#   select(condition, observed, predicted, exchange, f0, k, f_plateau, span1) %>%
#   head()
# 
# 
# 
