...
run_nls <- function(data, background_type, deadtime, debug = FALSE) {
  ...
  if (background_type == 'linear') {
    ...
    #### set starting values, upper bounds, and lower bounds for parameters (in order, f0, k, f_plateau, k_background)
    start <- list(f0 = max(data$observed, na.rm = T),
                  k = 0.001,
                  #c = median(data$observed, na.rm = T),
                  f_plateau = 0.6 * (max(data$observed, na.rm = T)),
                  k_background = 1e-5)
    
    upper <- c(1.2 * max(data$observed, na.rm = T),
               0.5,
               0.8 * (max(data$observed, na.rm = T)),
               #median(data$observed, na.rm = T), 
               1e-3) 
    
    lower <- c(0.8 * max(data$observed, na.rm = T),
               1e-6,
               min(data$observed, na.rm = T),
               1e-7)
    
    #### run non-linear least-squares regression with LM algorithm. format: out <- nlsLM(y ~ f(x))
    out <- nlsLM(observed ~ (f0 - (f_plateau - k_background*Time)) * exp(-k * Time) + (f_plateau - k_background * Time),
                 data = data, lower = lower, control = nls.lm.control(maxiter = 300))
    ...
    data$predicted <- (f0 - (f_plateau - k_background * data$Time)) * exp(-k * data$Time) + (f_plateau - k_background * data$Time)
  ...
  if (background_type == 'exponential') {
    ...
    # lower <- c(0.8 * max(data$observed, na.rm = T),
    #            1e-6,
    #            0.5 * min(data$observed, na.rm = T),
    #            0)
      
    #### run non-linear least-squares regression with LM algorithm. format: out <- nlsLM(y ~ f(x))
    # out <- nlsLM(observed ~ (f0 - f_plateau * exp(-k_background * Time)) * exp(-k * Time) + f_plateau * exp(-k_background * Time) + 1e3 * ((f0 - f_plateau)/f0 - 0.3) + 1e3 * (k - 0.1),
    #            data = data, start = start, control = nls.lm.control(maxiter = 300))
     out <- nlsLM(observed ~ (f0 - f_plateau * exp(-k_background * Time)) * exp(-k * Time) + f_plateau * exp(-k_background * Time),
               data = data, start = start, control = nls.lm.control(maxiter = 300))
    #out <- nlsLM(observed ~ (f0 - f_plateau * exp(-3e-5 * Time)) * exp(-k * Time) + f_plateau * exp(-3e-5 * Time) + 1e3 * ((f0 - f_plateau)/f0 - 0.3),
     #         data = data, start = start, control = nls.lm.control(maxiter = 300))
  ...
  if  (background_type == 'none') {
    ...
    start <- list(f0 = max(data$observed, na.rm = T),
                  k = 0.001,
                  f_plateau = median(data$observed, na.rm = T))
    ...
    data$predicted <- (f0 - f_plateau) * (exp(-k * data$Time)) + f_plateau
    data$exchange <- (f0 - f_plateau) * (exp(-k * data$Time)) + f_plateau
    data$background <- 0
  }
...
plot_data <- function(data, background_type, units) {
  ...
  pdf(paste(today, background_type, units, 'MM.pdf', sep= '_'), width = 10)
  print(plots)
  dev.off()
...
fit.parameters <- data.exp %>% 
  select(sample, conc, condition, f0, k, k_background, f_plateau, vf0, v0, f_signal_change) %>% 
  unique() %>%
  arrange(sample, conc)
ggplot(fit.parameters, aes(conc, v0, color = sample)) + geom_point()

data.exp <- plot_data(data.exp, background_type = 'exponential', units = 'fluor')
