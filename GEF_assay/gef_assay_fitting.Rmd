...
run_nls <- function(data, background_type, deadtime, debug = FALSE) {
  ...
  if (background_type == 'exponential') {
    ...
    start <- list(f0 = max(data$observed, na.rm = T),
                  f_plateau = 0.6*max(data$observed, na.rm = T),
                  f_background = min(data$observed, na.rm = T),
                  k = 5e-3,
                  k_background = 3e-5)
    
    upper <- c(2 * max(data$observed, na.rm = T),
               0.8 * max(data$observed, na.rm = T),
               2 * min(data$observed, na.rm = T),
               1e-1,
               1e-3)

    lower <- c(0.8 * max(data$observed, na.rm = T),
               0.3 * max(data$observed, na.rm = T),
    ...
    #### run non-linear least-squares regression with LM algorithm. format: out <- nlsLM(y ~ f(x))
     ...
     out <- nlsLM(observed ~ (f0 - f_plateau) * exp(-k * Time) + (f_plateau - f_background) * exp(-k_background * Time) + f_background, data = data, start = start, control = nls.lm.control(maxiter = 300))
