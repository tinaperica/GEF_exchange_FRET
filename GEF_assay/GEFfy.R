...
fit_MM <- function(data) { ### fits Michaelis-Menten
  ...
  kcat_Km_sd <- sqrt( (kcat_sd/kcat)^2 + (Km_sd/Km)^2 )
  data$kcat_Km_sd <- kcat_Km_sd
  data$predicted_v0 <- (data$conc * Vmax) / (Km + data$conc)
  return(data)
}   
fit_EH <- function(data) {   ### fits Edie-Hofstee plot v0 = Vmax - (Km * v0 / conc) (slope is -Km and intercept is Vmax)
  sample <- data$sample[1]
  print(sample)
  data_to_fit <- data %>% 
    select(condition, sample, conc, v0) %>% 
    unique() %>% 
    mutate("v_over_conc" = v0/conc)
  out <- lm(v0 ~ v_over_conc,
               data = data_to_fit)
  Vmax <- coef(out)[1]
  Km <- -1 * coef(out)[2]
  kcat <- Vmax
  data_to_fit %>% 
    ggplot(., aes(x = v_over_conc, y = v0, color = conc)) + 
    geom_point() +
    geom_abline(slope = - Km, intercept = Vmax)+
    scale_color_viridis() +
    theme_bw()
  data$kcat <- kcat
  data$Km <- Km
...
fit.parameters %>%
  #filter(date == "20190110") %>% 
  #mutate("nucleotide" = ifelse(row == "E", "mant-GTP", "mant_dGDP")) %>% 
...
# processed.data %>% 
#   group_by(condition) %>% 
#   mutate("max_flo" = max(observed)) %>% 
#   ungroup() %>% 
#   ggplot(aes(x = conc, y = max_flo)) + geom_point()
# 
# 
# fit.parameters %>%
#   ggplot(., aes(x = conc, y = span2, color = as.character(date))) + 
#   geom_point() + facet_grid(~ sample) + geom_abline(slope = fit$coefficients[2], intercept = fit$coefficients[1])
# 
# 
# fit <- lm(fit.parameters$f_plateau[fit.parameters$conc < 5] ~ fit.parameters$conc[fit.parameters$conc < 5])
# fit.parameters %>%
#   ggplot(., aes(x = conc, y = f_plateau, color = as.character(date))) + 
#   geom_point() + facet_grid(~ sample) + geom_abline(slope = fit$coefficients[2], intercept = fit$coefficients[1])
# 
...
MM.data %>% 
  #filter(sample != "PE16_R108I" & sample != "PE4_R108L" & sample != "PE18_R108Y" & sample != "PE26_R108Q"
   #      & sample != "PE29_R112S" & sample != "PE30_N105L") %>% 
  select(mutant, kcat_Km, kcat_Km_sd) %>%
  unique() %>% 
  mutate("mutant" = factor(mutant, mut_ordered_by_kcat_Km)) %>% 
  arrange(mutant) %>% 
  ggplot(aes(mutant, kcat_Km)) + geom_bar(stat = "identity") + 
...
MM.data %>% 
  select(mutant, kcat, Km, kcat_Km_sd) %>% 
  unique() %>% 
  ggplot(aes(x = kcat, y = log(Km), label = mutant)) + 
  geom_point(aes(size = kcat_Km_sd), alpha = 0.5) + 
  geom_text_repel() +
  ylab(label = "ln(Km)") +
