...
data<-read.delim("GEF_assay/data/WT_201711_kinetics.txt", stringsAsFactors = F, head = F)
...
head(data)
data <- data[data$conc > 0 & data$GEF_conc > 0,]
...
get_linear_region <- function(data) {
  ...
  reaction_percentages <- seq(0.10, 0.3, 0.05)
  points_check <- TRUE
...
plot <- ggplot(fitting.parameters, aes(x = conc, y = v0)) + geom_point()
print(plot)
