...
run_nls <- function(data, background_type, deadtime) {
  ...
  data <- rbind(data, data.frame(Time = extrapolated_time, fluorescence = NA, GEF_conc = data$GEF_conc[1]))
  if (background_type == 'linear') {
    ...
    data$predicted <- (f0 - c) * exp(-k * extrapolated_time) + c + k_background * extrapolated_time
    #data$signal <- (f0 - c) * exp(-k * data$Time) + c
    data$signal <- (f0 - c) * exp(-k * extrapolated_time) + c
    #data$background <- c + k_background * data$Time
    data$background <- c + k_background * extrapolated_time
  ...
  if  (background_type == 'exponential') {
    ...
    data$predicted <- (f0 -c) * (exp(-k * extrapolated_time) + exp(-k_background * extrapolated_time)) + c
    #data$signal <- (f0 - c) * (exp(-k * data$Time)) + c
    data$signal <- (f0 - c) * (exp(-k * extrapolated_time)) + c
    #data$background <- exp(-k_background * data$Time) + c
    data$background <- exp(-k_background * extrapolated_time) + c
    #data$min.fluorescence = data$signal[length(data$signal)]
    data$min.fluorescence <- min(data$signal, na.rm = T)
  ...
  if  (background_type == 'none') {
    ...
    cuttoff_time_range <- c(data$cutoff_time - 20, data$cutoff_time + 20)
...
```{r}
plot_data <- function(data, decay_type, units) {
  ...
  pdf(paste(Sys.Date(), decay_type, units, 'data.pdf', sep= '_'), width = 10)
  print(plots)
  dev.off()
  ...
  pdf(paste(Sys.Date(), decay_type, units, 'MM.pdf', sep= '_'), width = 10)
  print(plots)
  dev.off()

  # plot parameters for each fit
  pdf(paste(Sys.Date(), decay_type, units, 'params.pdf', sep= '_'))
  hist(unique(data$f0), breaks=100)
  hist(unique(data$k), breaks=100)
  hist(unique(data$c), breaks=100)
  hist(unique(data$v0), breaks=100)
  if (decay_type != 'none') {
    hist(unique(data$k_decay), breaks=100)
    print(ggplot(data, aes(x = conc, y = k_decay, color=sample)) + geom_point())
  }
  dev.off()
...
data.exp <- data.exp %>%
  ...
  do(run_nls(., decay_type='exponential')) %>%
  # plot_data(decay_type='exponential', units='fluor') %>%
  mutate(observed = conc - conc*(max.fluorescence - fluorescence)/(max.fluorescence - min.fluorescence)) %>%
  do(run_nls(., decay_type='exponential')) %>%
  plot_data(decay_type='exponential', units='conc')
```
...
data.lin <- data.lin %>%
  group_by(condition) %>%
  mutate(observed = fluorescence) %>%
  do(run_nls(., decay_type='linear')) %>%
  plot_data(decay_type='linear', units='fluor') %>%
  mutate(observed = conc - conc*((max(fluorescence, na.rm=T) - fluorescence)/(max(fluorescence, na.rm=T) - min.fluorescence))) %>%
  do(run_nls(., decay_type='linear')) %>%
  plot_data(decay_type='linear', units='conc')
```
...
data.nodecay <- data.nodecay %>%
  ...
  mutate(observed = fluorescence) %>%
  do(run_nls(., decay_type='none')) %>%
  plot_data(decay_type='none', units='fluor') %>%
  mutate(observed = conc - conc*((max(fluorescence, na.rm=T) - fluorescence)/(max(fluorescence, na.rm=T) - min.fluorescence))) %>%
  do(run_nls(., decay_type='none')) %>%
  plot_data(decay_type='none', units='conc')
```
