...
setwd("GEF_assay/")
data<-read.delim("data/GEF_FRET_kinetics.txt", stringsAsFactors = F, head = F)
...
get_region_to_fit <- function (data) {
  plateau_fluorescence <- quantile(data$fluorescence, prob = .8, na.rm = T)
...
get_linear_region <- function(data) {
  ...
  if (points_check == F) { ### this means the condition from above was passed at some point
...
fit_linear_rate <- function (data, prot, conc, GEF_conc, exp) {
  ...
  if (linear.subset != F) {
   ...
   # return(data.frame(prot, conc, "rate constant" = 0, "v0" = 0, GEF_conc, exp))
  #}
}
...
fit_MM <- function (initial_vs_conc, prot, GEF_conc) {
  ...
  kcat = Vmax / GEF_conc*0.001
  standard_errors_of_parameters <- round(summary(nls.out)$coefficients[, 2], 3)
  kcat_error <- round(as.numeric(standard_errors_of_parameters[[1]]), 3) / GEF_conc * 0.001
...
# 
#opar <- par()
filename <- paste0(Sys.Date(), "kcat_Km_GEF_exchange.pdf")
pdf(filename)
op<-par(mfrow=c(2,1))
kcat.barplot <- barplot(height = KmVm.param.df$kcat, names.arg = KmVm.param.df$prot,
                        ...
                        xaxt = "n", cex.names = 0.75, ylim = c(0,0.005))
...
Km.barplot <- barplot(height = KmVm.param.df$Km, names.arg = KmVm.param.df$prot,
                      ...
                      xaxt = "n", cex.names = 0.75, ylim = c(0,15))
