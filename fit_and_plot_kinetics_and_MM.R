...
get.initial.rate.subset <- function (total, conc) {
  ...
  if (conc < 1) {
    ...
    #cutoff.rxn.start.time <- max(total$time[total$percent.norm.fluorescence < 0.9])
  } else {
    cutoff.rxn.start.time <- max(total$time[total$percent.norm.fluorescence < 0.6])  # 0.8
    #cutoff.rxn.start.time <- max(total$time[total$percent.norm.fluorescence < 0.9])
  }
  cutoff.rxn.start.percent <- subset(total, time < cutoff.rxn.start.time)
  return(cutoff.rxn.start.percent)
}
fit_rate_and_plot <- function (initial, prot, conc, GEF_conc, exp) {
  time <- initial$time
  ...
  for (conc in substr.concentrations) {
    ...
    for (GEF_conc in GEF_concentrations) {
      ...
      for (exp in experiments) {
        ...
        fitting.parameters <- rbind(fitting.parameters, fit_rate_and_plot(relevant.subset, prot, conc, GEF_conc, exp))
      }
    }
  }
  dev.off()
...
for (i in 1:length(proteins)) {
  mutant = proteins[i]
  data.subset <- subset(fitting.parameters, prot == mutant, select=c(conc = conc, v0 = v0, GEF_conc = GEF_conc, exp = exp))
